...
class Client extends core.Client {
  ...
  Client(String apiKey, blog): super(apiKey, blog is Uri ? blog : Uri.parse(blog)) {
    userAgent='Dart/1.0.0 | Akismet/${core.VERSION}';
  }
  ...
  Future<String> _queryService(Uri endPoint, Map<String, String> fields) {
    ...
    return HttpRequest.postFormData(endPoint.toString(), fields, requestHeaders: headers).then((request) {
      ...
      if(request.responseHeaders.containsKey('x-akismet-debug-help'))
        throw new Exception(request.responseHeaders['x-akismet-debug-help'] /*, uri: endPoint */); // TODO: HttpException
    ...
    /* TODO
    return HttpRequest.getString(endPoint.toString()).then((response) {
      print(response);
      return response;
    });
    */
